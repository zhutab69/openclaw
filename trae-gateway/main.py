...
async def lifespan(app: FastAPI):
    ...
    # Load Trae builtin models (no API fetch needed)
    # Trae does not provide a model list API - models are builtin to the IDE
    logger.info("Loading Trae builtin models...")
    await app.state.model_cache.update(FALLBACK_MODELS)
    logger.debug(f"Loaded {len(FALLBACK_MODELS)} Trae builtin models")
    
    # Log final cache state
    all_models = app.state.model_cache.get_all_model_ids()
    logger.info(f"Model cache ready: {len(all_models)} Trae models total")
    
    # Create model resolver (uses cache for resolution)
    app.state.model_resolver = ModelResolver(
        cache=app.state.model_cache,
        hidden_models={},  # Trae doesn't use hidden models
        aliases={},  # Trae doesn't use aliases
        hidden_from_list=[]  # Trae doesn't hide models
    )
